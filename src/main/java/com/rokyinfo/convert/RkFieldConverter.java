    ...
    public static <T> T bytes2entity(T entity, byte[] bytes) throws IllegalAccessException, RkFieldException, FieldConvertException {
        ...
        for (Field field : fields) {
            ...
            if (mRkField != null) {
                ...
                int bitPosition = mRkField.bitPosition();
                int bitLength = mRkField.bitLength();

                if((bitPosition + bitLength) > value.length * 8){
                    throw new FieldConvertException("已超出属性的bit位长度" + field.getName());
                }

                if(bitPosition >= 0 && bitLength > 0){

                    int fieldValue = ByteConvert.get(value, bitPosition, bitLength);
                    value = ByteConvert.intToBytes(fieldValue);
